...
- `IJobRegistry` (`contracts/interfaces/IJobRegistry.sol`): See classified selectors; add typed owner override before PASS if no recovery selector exists for a nonterminal state.
- `IProofSubmissionRegistry` (`contracts/interfaces/IProofSubmissionRegistry.sol`): See classified selectors; add typed owner override before PASS if no recovery selector exists for a nonterminal state.
- `IReputationRegistry` (`contracts/interfaces/IReputationRegistry.sol`): See classified selectors; add typed owner override before PASS if no recovery selector exists for a nonterminal state.
