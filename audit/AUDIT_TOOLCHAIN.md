...
- `npm run audit:slither` emits `slither.txt`, `slither.json`, and `slither.sarif` where supported.
...
- `npm run audit:semgrep` runs Semgrep rules for workflows/scripts/secrets/deploy controls or records environment-blocked evidence.
- `npm run audit:npm` runs `npm audit --json` and writes a dependency triage summary.
