...
def build_page(content: dict[str, Any], manifest: dict[str, Any], records: list[dict[str, Any]], route: list[dict[str, Any]]) -> str:
    ...
    route_html = "".join(
        f"<article class='pg-route-node'><span class='pg-route-number'>{x['sequence']:02d}</span><div><small>{esc(x['stage']).upper()}</small><h3>{esc(x['contractName'])}</h3><code>{esc(x['address'])}</code><p>{esc(x['functionSignature'])}</p></div><a href='{esc(x['etherscanUrl'])}' target='_blank' rel='noopener noreferrer'>Etherscan ↗</a></article>"
        for x in route
    )
    filter_buttons = "".join(
    ...
    contract_cards = "".join(
        f"<article class='pg-contract' data-category='{esc(x['category'])}' data-search='{esc((x['name'] + ' ' + x['address'] + ' ' + x['purpose']).lower())}'><div class='pg-contract-top'><span class='pg-tag'>{esc(x['category'].upper())}</span><a href='{esc(x['etherscanUrl'])}' target='_blank' rel='noopener noreferrer'>Etherscan ↗</a></div><h3>{esc(x['name'])}</h3><p>{esc(x['purpose'])}</p><code>{esc(x['address'])}</code></article>"
        for x in created
    )
...
<footer class='pg-footer'><div class='pg-wrap'><div><strong>GoalOS AGIALPHA Ascension</strong><br><small>Proof-governed autonomous work.</small></div><div><a href='{esc(content['repositoryUrl'])}' target='_blank' rel='noopener noreferrer'>GitHub repository ↗</a></div></div></footer>
<script>
