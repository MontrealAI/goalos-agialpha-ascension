...
r = run(['setup','--network','mainnet','--json']); assert.strictEqual(r.status,0,r.stderr);
r = run(['doctor','--network','mainnet','--json']); assert.notStrictEqual(r.status,0); assert.match(r.stdout,/"status": "BLOCKED"/); assert.match(r.stdout,/PRIVATE_MAINNET_RPC_URL|AGIALPHA_TOKEN_ADDRESS/);
