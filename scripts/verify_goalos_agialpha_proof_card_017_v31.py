...
def main():
    ...
    required=['index.html','proof-cards.html','agialpha-ledger-route.html','sovereign-rsi-control-plane.html','evidence-docket.html']+[f'proof-card-{i:03d}.html' for i in range(1,18)]
    ...
    pc17=(site/'proof-card-017.html').read_text(encoding='utf-8',errors='replace')
    for marker in ['Everyday Billion-User Utility Engine','regular-person tools','privacy-safe','referral','retention','AEPPersonalUtilityRegistry','AEPUserNeedGraphRegistry','large specialist-agent']:
        if marker not in pc17: fail(f'proof-card-017 missing {marker}')
    ...
    print('Main website Proof Cards 001-017 verification passed.')
