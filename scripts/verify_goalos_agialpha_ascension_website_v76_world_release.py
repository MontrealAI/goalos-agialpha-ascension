#!/usr/bin/env python3
"""Verify the GoalOS AGIALPHA Ascension Website v76 World Release Pages artifact."""
...
REQUIRED_PROOF_CARD_028_TEXT = "The civilizational prize is not ownership of a mythical machine. It is verified capability that compounds"
LEGACY_PROOF_CARD_028_RE = re.compile(r"A superintelligent machine would be.{0,160}immense value", re.I | re.S)
DENYLIST = [
    "achieved AGI",
    "achieved ASI",
...
def main() -> int:
    ...
    if not site.exists():
        ...
        pc028 = (site / "proof-card-028.html").read_text(encoding="utf-8", errors="ignore") if (site / "proof-card-028.html").exists() else ""
        pc028_plain = strip_tags(pc028)
        if REQUIRED_PROOF_CARD_028_TEXT not in pc028_plain:
            errors.append("Proof Card 028 is missing the GoalOS-native reformulation")
        if LEGACY_PROOF_CARD_028_RE.search(pc028_plain):
            errors.append("Proof Card 028 still contains the legacy superintelligent-machine quote")

        all_public_text = "\n".join(f.read_text(encoding="utf-8", errors="ignore") for f in site.rglob("*") if f.is_file() and f.suffix.lower() in TEXT_SUFFIXES)
        if LEGACY_PROOF_CARD_028_RE.search(strip_tags(all_public_text)):
            errors.append("public site still contains the legacy superintelligent-machine quote")

        qa_json = site / "qa" / "QA_ASCENSION_WEBSITE_v76_WORLD_RELEASE.json"
        if not qa_json.exists():
            errors.append("QA manifest missing: qa/QA_ASCENSION_WEBSITE_v76_WORLD_RELEASE.json")
        else:
            try:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("Verification passed for GoalOS AGIALPHA Ascension Website v76 World Release.")
    return 0
