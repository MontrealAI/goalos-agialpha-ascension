...
def main():
    ...
    for term in ["GoalOS Mission OS","The Proof OS for Autonomous AI Work","mission-builder","Mission OS Packet Builder","Verifier","Chronicle","AGIALPHA","Evidence Docket","RSI","mission-os-v48-visual-contrast-hardening"]:
        if term not in mission: fail(f"mission-os missing {term}")
    if "mission-os-v48-anchor-fix" not in mission and "mission-os-v47-anchor-fix" not in mission: fail("mission builder anchor fallback missing")
    if "world-section.dark td" not in mission or "color:#071427" not in mission: fail("dark-section table contrast hardening missing")
    ...
    print("GoalOS AGIALPHA final main website v48 verification passed.")
