...
def source_identity_certificate():
 path=ROOT/'qa/mainnet-source-identity/source-identity-certificate.json'
 if not path.exists():
  return {'classification':'SOURCE_IDENTITY_NOT_PROVEN','status':'MISSING','exactReproductionProven':False,'certificateHash':None,'blockers':['source identity certificate missing']}
 data=json.load(open(path,encoding='utf-8'))
 return {
  'classification': data.get('classification','SOURCE_IDENTITY_NOT_PROVEN') if data.get('status')=='PASS' else 'SOURCE_IDENTITY_NOT_PROVEN',
  'status': data.get('status','MISSING'),
  'exactReproductionProven': bool(data.get('exactReproductionProven')) and data.get('status')=='PASS',
  'historicalDeploymentCommitProven': bool(data.get('historicalDeploymentCommitProven')),
  'reproductionSourceCommitSha': data.get('reproductionSourceCommitSha'),
  'certificateHash': data.get('certificateHash'),
...
def source_doc():
 m,state,sha,pkg=evidence(); sid=source_identity_certificate(); created=[c for c in m['contracts'] if c.get('classification')=='deployed']
 text=f"""# Ethereum Mainnet source identity — 2026-06-21\n\n## Result\n\nFinal identity classification: `{sid['classification']}`.\n\nThis repository commit is a release-preparation candidate, but exact deployed source identity is not asserted by this document because the historical deployment manifest records `commit = {m.get('commit')}` and live creation-bytecode/source comparisons have not been completed in this environment. Runtime equivalence and creation-input identity are derived only from `qa/mainnet-source-identity/source-identity-certificate.json`; if that certificate is not PASS, this document remains fail-closed.\n\n## Inputs\n\n- Package version: `{pkg['version']}`.\n- Deployment-script historical label: `{m.get('package')}`.\n- Candidate Git SHA: `{sha}`.\n- Network / chain ID: `{m.get('network')}` / `{m.get('chainId')}`.\n- Compiler: Solidity `0.8.28`.\n- Hardhat: `{pkg['devDependencies'].get('hardhat')}`.\n- Deployment timestamp: `{m.get('deployedAt')}`.\n\n## Comparison coverage\n\n- Runtime-bytecode comparison result: `{sid['status']}` for the source-identity certificate.\n- Creation-bytecode comparison coverage: `{sid['status']}`; constructor-input and creation-code reconstruction are asserted only when the certificate is PASS.\n- Etherscan verified-source comparison: repository evidence reports Mainnet verification, but exact source-to-commit identity remains pending independent validation.\n- Metadata-only differences: none asserted; any differences must be documented after bytecode and Etherscan source comparison.\n\n## Claim boundary\n\nThe published pre-release tag exists as a deployment record. Exact deployed-source reproducibility is claimed only when the source-identity certificate status is `PASS`. Certificate hash: `{sid.get('certificateHash')}`. Blockers: `{'; '.join(sid.get('blockers') or []) or 'none'}`.\n"""
...
def prepare():
 ...
 manifest={'releaseTag':TAG,'releaseTitle':TITLE,'targetGitSha':sha,'requestedRef':os.getenv('RELEASE_REQUESTED_REF') or os.getenv('GITHUB_REF_NAME') or 'LOCAL_HEAD','sourceIdentityClassification':source_identity_certificate()['classification'],'chainId':1,'deployedAt':m['deployedAt'],'configuredAt':m['configuredAt'],'walletA':m['walletA'],'walletB':m['walletB'],'canonicalAgialpha':m['agialphaToken'],'goalosContractAddresses':[c['address'] for c in created],'deploymentTransactionHashes':txs,'phaseBGrants':m['phaseBGrants'],'compilerSettings':{'solidity':'0.8.28','hardhat':pkg['devDependencies'].get('hardhat')},**roots,'productionActivated':False,'userFundsAuthorized':False,'externallyAudited':False,'releaseAssetHashes':{},'releaseVerificationReadiness':'DRAFT_BLOCKED_PENDING_INDEPENDENT_REVALIDATION'}
