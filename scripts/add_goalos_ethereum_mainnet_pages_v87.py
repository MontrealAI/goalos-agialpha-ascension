...
@media(max-width:700px){{.mn-container{{width:min(100% - 24px,1180px)}}.mn-hero{{padding-top:62px;border-radius:0 0 34px 34px}}.mn-hero h1{{font-size:clamp(3rem,16vw,5rem)}}.mn-metrics,.mn-boundaries,.mn-contract-grid{{grid-template-columns:1fr}}.mn-registry-intro{{display:block}}.mn-registry-chip{{margin-top:16px}}.mn-group summary{{grid-template-columns:38px 1fr 16px}}.mn-group-count{{grid-column:2;font-size:.72rem}}.mn-group-chevron{{grid-column:3;grid-row:1 / span 2}}.mn-contract-item a{{align-items:flex-start;flex-direction:column}}.mn-address{{align-items:flex-start;flex-direction:column;width:100%}}.mn-address code{{width:100%;max-width:100%}}.mn-footer-inner{{display:block}}.mn-footer-inner>div+div{{margin-top:14px}}}}
