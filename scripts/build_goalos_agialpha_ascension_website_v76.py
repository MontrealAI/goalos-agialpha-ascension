#!/usr/bin/env python3
"""Build GoalOS AGIALPHA Ascension Website v76 into a static GitHub Pages site."""
...
RELEASE = "GoalOS AGIALPHA Ascension Website v76 — Platinum Ascension Command / AGI Alpha Continuity Gold Master"
VERSION = "v76"
...
{claim_boundary_html()}
"""
    return layout("GoalOS AGIALPHA Ascension — Website v76", "Turn AI work into verified capability. GoalOS creates proof; $AGIALPHA makes accepted proof economically consequential.", body)
...
def pages() -> dict[str,str]:
    ...
    pg["proof-run-001.html"]=simple_page("proof-run-001","Proof Run 001","The first public Evidence Docket threshold: one real objective, one governed decision state, one replay path.",[
        '<section class="section"><div class="wrap"><p class="kicker">Evidence threshold</p><h2>Proof Run 001 turns Ascension from doctrine into public evidence.</h2><p class="lead">The claim to prove is precise: GoalOS Mission OS can take one real objective and produce a governed decision state with claims, evidence, risks, verifier report, action graph, Chronicle entry, claim boundary, and replay path.</p><div class="run-gate"><div class="run-step">Objective</div><div class="run-step">Mission Contract</div><div class="run-step">Agent Theater</div><div class="run-step">Evidence Docket</div><div class="run-step">Verifier Report</div><div class="run-step">Decision State</div><div class="run-step">Chronicle</div></div></div></section>',
        '<section class="section"><div class="wrap"><h2>Acceptance gate.</h2><div class="table-wrap"><table><tr><th>Required artifact</th><th>Why it matters</th></tr><tr><td>Claims Matrix</td><td>Separates claim, evidence, uncertainty, and non-claim.</td></tr><tr><td>Source Provenance</td><td>Shows where evidence came from and what can be checked.</td></tr><tr><td>Contradiction Register</td><td>Prevents a persuasive answer from hiding uncertainty.</td></tr><tr><td>Risk Ledger</td><td>Records safety, legal, security, privacy, and rollout risks.</td></tr><tr><td>Verifier Report</td><td>Decides whether the proof can be accepted, challenged, or rejected.</td></tr><tr><td>Action Graph</td><td>Turns evidence into practical next steps.</td></tr><tr><td>Chronicle Entry</td><td>Makes accepted work reusable for future missions.</td></tr></table></div></div></section>'
    ],"proof-run-001.html")
...
def write_site_files(out: Path, copied_assets: dict, page_map: dict[str,str]) -> None:
    ...
    status={
        ...
        "proof_run_001_status": "next_required_public_evidence_docket",
