...
async function writeReport(report: any) { fs.mkdirSync("docs", { recursive: true }); fs.writeFileSync("docs/AGIALPHA_TOKEN_VERIFICATION_REPORT.md", `# AGIALPHA Token Verification Report\n\nStatus: **${report.status}**\n\n\`\`\`json\n${JSON.stringify(report, null, 2)}\n\`\`\`\n`); }
async function main() {
  ...
  const report={status:"VERIFIED_READ_ONLY", address:AGIALPHA_MAINNET, name, symbol, decimals:Number(decimals), totalSupply:totalSupply.toString(), mainnetAuthorized:false};
