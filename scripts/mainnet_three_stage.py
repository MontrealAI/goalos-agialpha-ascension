...
def predeploy_certificate():
 ...
 c={'schemaVersion':'1.0','stage':'A_PREDEPLOYMENT_AUTHORIZATION',**release_identity(),'protectedEvidenceIndexHash':validation.get('indexSha256'),'forkBlockNumber':fork.get('forkBlockNumber'),'forkBlockHash':fork.get('forkBlockHash'),'forkEvidenceRoot':sha('qa/mainnet-predeploy/fork-rehearsal.json'),'sepoliaRoot':validation.get('summary',{}).get('sepolia'),'deploymentPlanHash':plan.get('planHash'),'gateReportHashes':{g:sha(GATES/GATE_SPECS[g][0]/'report.json') for g in GATE_SPECS},'status':status,'blockers':blockers,'gates':gates,'TECHNICALLY_MAINNET_READY':yes,'MAINNET_DEPLOYMENT_AUTHORIZED':yes,'ETHEREUM_MAINNET_AUTHORIZED':yes,'MAINNET_DEPLOYED':'NO','MAINNET_VERIFIED':'NO','LIVE_AUTHORITY_READBACK_COMPLETE':'NO','LIVE_OWNER_HANDOFF_COMPLETE':'NO','LIVE_CANARY_COMPLETE':'NO','PRODUCTION_ACTIVATION_EFFECTIVE':'NO','expiresAt':expiry,'evidence':[evidence_ref('qa/mainnet-predeploy/fork-rehearsal.json'),evidence_ref('qa/mainnet-predeploy/deployment-plan.public.json'),evidence_ref('qa/mainnet-predeploy/evidence/protected-evidence-validation.json')],'mainnetBroadcastOccurred':False}
