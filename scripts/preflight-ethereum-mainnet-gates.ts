...
const bytes32Keys = ["LEGAL_SIGNOFF_HASH","TAX_SIGNOFF_HASH","SECURITY_REVIEW_HASH","PUBLIC_CLAIMS_REVIEW_HASH","TREASURY_REVIEW_HASH","AGIALPHA_TOKEN_VERIFICATION_HASH","SEPOLIA_REHEARSAL_EVIDENCE_HASH","EXTERNAL_AUDIT_CLOSURE_HASH","FOUNDER_APPROVAL_HASH"];
...
if (process.env.ALLOW_MAINNET_DEPLOYMENT !== "YES_ALL_GATES_APPROVED") blockers.push("ALLOW_MAINNET_DEPLOYMENT must equal YES_ALL_GATES_APPROVED only after all real gates are complete");
...
if (!process.env.ETHEREUM_MAINNET_RPC_URL) blockers.push("ETHEREUM_MAINNET_RPC_URL is required for real read-only mainnet verification");
console.log(JSON.stringify({ status: blockers.length ? "NOT_AUTHORIZED" : "PREFLIGHT_PASSED_READ_ONLY", blockers, mainnetDeployment: "not executed" }, null, 2));
